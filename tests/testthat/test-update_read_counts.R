test_that("update_read_counts returns same length vector as input", {
  counts <- c(10, 5, 3)
  sim    <- diag(3)
  result <- update_read_counts(counts, sim, cutoff = 0.8)
  expect_equal(length(result), 3)
})

test_that("update_read_counts leaves counts unchanged when no neighbors exceed cutoff", {
  counts <- c(10, 5, 3)
...
test_that("update_read_counts incorporates neighbor counts above cutoff", {
...
test_that("update_read_counts returns 0 for zero-count sequence with no similar neighbors", {
  counts <- c(5, 0)
...
test_that("update_read_counts respects the cutoff threshold", {
  counts <- c(10, 8)
...
test_that("update_read_counts weighted average is correct for two equal sequences", {
  counts <- c(8, 4)
